...
export const GoalForm = ({ goal, initialStudentId, onSuccess, onCancel }: GoalFormProps) => {
  const [studentId, setStudentId] = useState(goal?.studentId ?? initialStudentId ?? '');
  const [authorId, setAuthorId] = useState(goal?.authorId ?? '');
  const [selectedSubjects, setSelectedSubjects] = useState<string[]>(goal?.subjectIds ?? []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const targetDateTimestamp = targetDate ? new Date(targetDate).getTime() : undefined;
      ...
      if (goal) {
        await updateGoal({
          ...
          note,
          status: status as "NOT_STARTED" | "IN_PROGRESS" | "COMPLETED" | "ON_HOLD" | "CANCELLED",
  ...
  return (
    <Card className="max-w-2xl mx-auto bg-gray-800 border-gray-700">
      <CardHeader>
        <CardTitle className="text-white">{goal ? 'Edit Goal' : 'Add Goal'}</CardTitle>
      </CardHeader>
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-2 max-h-32 overflow-y-auto bg-gray-700 border border-gray-600 rounded-md p-3">
              {subjects?.map((subject) => (
                <div key={subject._id} className="flex items-center space-x-2">
                  <Checkbox
                    id={subject._id}
                    checked={selectedSubjects.includes(subject._id)}
                    onCheckedChange={(checked) => handleSubjectChange(subject._id, checked === true)}
                    disabled={isLoading}
                  />
                  <Label htmlFor={subject._id} className="text-sm text-gray-300">
                    {subject.name}
                  </Label>
                </div>
              ))}
            ...
            {selectedSubjects.length === 0 && (
              <p className="text-sm text-red-400">Please select at least one subject</p>
            )}
          </div>

          <div className="space-y-2">
            <Label htmlFor="note" className="text-gray-300">Goal Description</Label>
            <Textarea
              id="note"
              value={note}
          ...
          <div className="flex justify-end space-x-2">
            ...
            <Button 
              type="submit" 
              disabled={isLoading || !note || selectedSubjects.length === 0 || (!goal && (!studentId || !authorId))} 
