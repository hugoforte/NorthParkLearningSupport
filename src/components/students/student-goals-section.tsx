...
import { Target, Plus, Edit, Trash2, CheckCircle, Circle, X } from 'lucide-react';
...
const statusOptions = [
  { value: "NOT_STARTED", label: "Not Started", color: "bg-gray-600", icon: Circle },
  { value: "IN_PROGRESS", label: "In Progress", color: "bg-blue-600", icon: Target },
...
export const StudentGoalsSection = ({ studentId, goals }: StudentGoalsSectionProps) => {
      ...
      <CardContent>
        {goals.length === 0 ? (
          <div className="text-center py-8">
            ...
            <p className="text-gray-400 mb-4">No goals set for this student yet.</p>
            <Link href={`/goals/add?studentId=${studentId}`}>
          ...
          <div className="space-y-4">
            ...
            {activeGoals.length > 0 && (
              <div>
                <h4 className="text-sm font-medium text-gray-300 mb-3">Active Goals</h4>
                <div className="space-y-3">
                  {activeGoals.map((goal) => {
                    ...
                    return (
                      <div key={goal._id} className="bg-gray-700 rounded-lg p-4">
                        ...
                        <p className="text-white text-sm mb-2">{goal.note}</p>
                        <div className="flex items-center justify-between text-xs text-gray-400">
                          <span>Subjects: {getSubjectNames(goal.subjectIds)}</span>
                          <span>Target: {formatDate(goal.targetDate)}</span>
                        </div>
                        <div className="text-xs text-gray-500 mt-1">
                          By {getTeacherName(goal.authorId)} • {new Date(goal._creationTime).toLocaleDateString()}
                        </div>
                      </div>
                    );
                  })}
            ...
            {completedGoals.length > 0 && (
              <div>
                <h4 className="text-sm font-medium text-gray-300 mb-3">Completed Goals</h4>
                <div className="space-y-3">
                  {completedGoals.map((goal) => (
                    <div key={goal._id} className="bg-gray-700 rounded-lg p-4 opacity-75">
                      <div className="flex items-start justify-between mb-2">
                        <div className="flex items-center space-x-2">
                      ...
                      <p className="text-white text-sm mb-2 line-through">{goal.note}</p>
                      <div className="flex items-center justify-between text-xs text-gray-400">
                        <span>Subjects: {getSubjectNames(goal.subjectIds)}</span>
