...
import { FileText, Plus, Edit, Trash2, Lock, Eye, Calendar } from 'lucide-react';
...
export const StudentNotesSection = ({ studentId, notes }: StudentNotesSectionProps) => {
  ...
  const sortedNotes = [...notes].sort((a, b) => b._creationTime - a._creationTime);
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <CardTitle className="text-lg text-white">Notes & Observations</CardTitle>
              <p className="text-sm text-gray-400">
                {notes.length} total notes • {notes.filter(n => n.isPrivate).length} private
              </p>
            </div>
          </div>
      ...
      <CardContent>
        {notes.length === 0 ? (
          <div className="text-center py-8">
            ...
            <p className="text-gray-400 mb-4">No notes recorded for this student yet.</p>
            <Link href={`/notes/add?studentId=${studentId}`}>
          ...
          <div className="space-y-4">
            {sortedNotes.map((note) => {
              ...
              return (
                <div key={note._id} className="bg-gray-700 rounded-lg p-4">
                  ...
                  <p className="text-white text-sm mb-3 leading-relaxed">{note.content}</p>
                  
                  <div className="flex items-center justify-between text-xs text-gray-400">
                    <div className="flex items-center space-x-1">
                      <Calendar className="h-3 w-3" />
                      <span>{formatDate(note._creationTime)} at {formatTime(note._creationTime)}</span>
                    </div>
